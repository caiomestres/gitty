...
| D5 | Foundation split into 3 vertical slices; slice 1 = Discovery & Registry (demoable via CLI) | Milestone 1 too large for one slice | 2026-06-01 |
...
| D10 | Git write = shell-out runner `(path, args) -> {code, stdout, stderr}`; git located via PATH, validated at startup (slice 2) | ADR-0001 | 2026-06-01 |
| D11 | Lock = per-repo PID+timestamp lock file in config dir, stale detection, fail-fast on contention (slice 3) | ADR-0006 | 2026-06-01 |
...
| Manual re-link resolution UI (for ambiguous fingerprint matches) | Slice 1 auto-links only unambiguous matches | ADR-0005 |
| Submodule-aware discovery (flag/filter submodules) | Slice 1 treats nested repos generically | Grilling D1 |
...
- [x] Slice 1 — Discovery & Registry: config, repository, scan, git::read, re-link, CLI (scan/list/status) — all 12 DISC reqs verified
- [ ] Slice 2 — Git write layer (shell-out runner, fetch/pull)
- [ ] Slice 3 — Lock mechanism (per-repo PID lock files)
